...
class CollectionViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        for items in 0 ... 1 {
            ...
            for i in 0 ... 4 {
                ...
                if items == 0 {
                    ...
                    headerBtn.frame.origin.y = screenSpaced + navLabelFrame.maxY
        ...
        // 创建空的头部视图调整cell的位置
        let headerView = UIView(frame: CGRect(origin: CGPointZero, size: CGSize(width: screenWidth, height: headerBtnArray[0].frame.maxY)))
...
extension CollectionViewController: UITableViewDataSource, UITableViewDelegate {
    ...
    func scrollViewDidScroll(_ scrollView: UIScrollView) {
        ...
        // 导航栏初始大标题渐变消失和放大动画代码
        if collectionTableView.contentOffset.y >= 0 {
            self.navLabel.frame.origin.y = screenSpaced - collectionTableView.contentOffset.y
            self.navLabel.alpha = 1 - (collectionTableView.contentOffset.y / navHeight)
        } else {
            self.navLabel.frame.origin.y = screenSpaced - collectionTableView.contentOffset.y / 2.5
            self.navLabel.frame.size.height = screenHeight / 25 * (1 - collectionTableView.contentOffset.y / navHeight / 12)
            self.navLabel.font = UIFont.systemFont(ofSize: screenHeight / 25 * (1 - collectionTableView.contentOffset.y / navHeight / 12), weight: .bold)
            self.navLabel.alpha = 1 - (collectionTableView.contentOffset.y / navHeight)
        }
        ...
        if self.navLabel.alpha <= 0.7 {
            ...
            titleView.alpha = (collectionTableView.contentOffset.y / navHeight) - 0.36776
        } else {
